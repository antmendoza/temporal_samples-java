...
public class AutoWorkflowImpl implements AutoWorkflow {
  ...
  public String exec(String input) {
    ...
    AutoActivities activitiesFifth =
        Workflow.newActivityStub(
            AutoActivities.class,
            ActivityOptions.newBuilder()
                .setCancellationType(ActivityCancellationType.WAIT_CANCELLATION_COMPLETED)
    ...
    scope =
        Workflow.newCancellationScope(
            () -> {
              //              activityPromises.add(Async.function(activitiesOne::runActivityOne,
              // input));
              //            activityPromises.add(Async.function(activitiesTwo::runActivityTwo,
              // input));
              activityPromises.add(Async.function(activitiesThree::runActivityThree, input));
              // activityPromises.add(Async.function(activitiesFour::runActivityFour, input));
              activityPromises.add(Async.function(activitiesFifth::runActivityFifth, input));
              activityPromises.add(Async.function(activitiesSix::runActivitySix, input));
            });

    scope.run();

    Promise.anyOf(activityPromises).get();
    scope.cancel("Canceling fron inside the workflow");
    String result = "";
    for (Promise<String> pr : activityPromises) {
      try {
        // wait for all activity result or failure
        result += pr.get();
      } catch (Exception e) {
